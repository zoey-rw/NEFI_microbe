#plotting all top 20 species with same forecast_plot.
rm(list=ls())
source('paths.r')
...
#output path
figure.path <- 'figures/top20_prior_prediction.png'
...
obs <- d[,colnames(d) %in% names(mod$all.preds$species_parameter_output)]
...
#begin plotting.----
png(filename=figure.path,width=16,height=12,units='in',res=300)
par(mfrow = c(4,5))
