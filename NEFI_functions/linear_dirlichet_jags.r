...
#'
#' @return returns a list containing the jags model, a list of parameter tables by species, a list of predicted, observed, residuals by species on (0-1) scale.
#' @export
#'
#' @examples
#' #load tedersoo data.
...
#' 
#' #Run function.
#' linear_dirlichet_jags(x_mu=x_mu, x_sd=x_sd, y=y, z.trans = T, sample = 800)

linear_dirlichet_jags <- function(y, x_mu, x_sd, z.trans = F, n.chains = 3, adapt = 100, burnin = 200, sample = 400, parallel = F){
  ...
  pred.list <- list()
  for(i in 1:ncol(y)){
    pred <- exp(as.matrix(x_mu) %*% as.numeric(as.character(output.list[[i]][,4])))
    pred.list[[i]] <- pred
  }
