...
# Specify output path
output.path <- dp1.10098.001_plot.level_output.path

#load tree data
dat <- readRDS(dp1.10098.001_output.path)

# # subset to data from 2017 or before
# dat <- dat[dat$dateID <= 2017,]
# # 2017 cutoff is because some sites didn't have data till then
# # but we don't want to include 2018/2019 if we don't have to
# oursites <- list("ORNL","WOOD","CPER","TALL","JERC","OSBS","HARV","BART","STER","SCBI","DSNY","UNDE")
# length(intersect(unique(dat$siteID), oursites))
# # nevermind - including recent data gives us more plot-level data
