...
#try creating missing data!
#core_mu[c(2:4,18),2] <- NA
#plot_mu[3,1] <- NA
#site_mu[2,1] <- NA
...
jags.data <- list(N.core = nrow(core_mu), N.plot = nrow(plot_mu), N.site = nrow(site_mu), N.spp = ncol(y),            #number of c/p/s observations and number of species
                  ...
                  core_plot = core_plot, core_site = core_site, plot_site = plot_site, plot_plot = plot_plot, site_site = site_site,         #vectors that index across  c/p/s
...
### Account for uncertainty in predictor values ###
