...
#' @param core_sd core level obsercation sd as a dataframe. column names should match core_mu. optional.
#' @param plot_sd plot level obsercation sd as a dataframe. column names should match plot_mu. optional.
#' @param site_sd site level obsercation sd as a dataframe. column names should match site_mu. optional.
...
#'
#' @examples
hierarchical_dirlichet_jags.r <- function(y,
                                          core_mu, plot_mu, site_mu,
                                          core_plot, core_site, plot_plot, site_site,
  ...
  pred.list <- list()
  for(i in 1:ncol(y)){
    pred <- exp(as.matrix(super.x) %*% as.numeric(as.character(output.list[[i]][,4])))
    pred.list[[i]] <- pred
  }
