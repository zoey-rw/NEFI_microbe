...
seq.dir <- paste0(big_data_dir, 'NEON_2014-2017_seqs/ITS/')	
# set output for ESV table and tracking path.
esv.table.path <- paste0(seq.dir, 'dada2_output/SV.table.rds')
track.path <- paste0(seq.dir, 'dada2_output/track.table.rds')

#get demultiplexed/joined fastq files, read in the file names.
read.motif <- '.fastq'
reads <- sort(list.files(paste0(seq.dir, 'raw_fastq_ITS/'), pattern = read.motif, full.names = T))
