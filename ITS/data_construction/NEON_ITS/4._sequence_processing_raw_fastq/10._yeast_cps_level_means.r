#getting site level means of NEON cosmopolitan genera for forecast validation.
rm(list=ls())
library(runjags)
source('paths.r')

#set output.path----
plot.output.path <- NEON_plot.level_yeast_obs_fastq.path
site.output.path <- NEON_site.level_yeast_obs_fastq.path

#load data and format.----
d <- readRDS(NEON_ITS_fastq_yeast_taxa.path)
fg.c <- d$abundances
