#plotting yeast forecasts vs. validation data.
#clear environment, source paths, functions and packages.
rm(list=ls())
source('paths.r')

#set output path.----
output.path <- NEON_dmulti.ddirch_yeast_forecast_validation.path
output.path <- 'test.png'

#load data.----
d <- readRDS(NEON_dmulti.ddirch_fcast_yeast.path)
core.truth <- readRDS(NEON_ITS_fastq_yeast_taxa.path)
core.truth <- core.truth$rel.abundances
rownames(core.truth) <- gsub('-GEN','',rownames(core.truth))
plot.truth <- readRDS(NEON_plot.level_yeast_obs_fastq.path)
site.truth <- readRDS(NEON_site.level_yeast_obs_fastq.path)
