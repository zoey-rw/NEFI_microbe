...
truth <- readRDS(NEON_cosmo_abundances.path) #UPDATE THIS FOR GENERA.
...
#plot
plot(obs.mu ~ mu, cex = 0.7, ylim=c(0,.2), main = 'core-level')
...
#obs.hi95 <- fghi95[,i][order(match(names(fghi95[,i]),names(mu)))]
#plot
plot(obs.mu ~ mu, cex = 0.7, ylim=c(0,.05), main = 'core-level')
rsq <- round(summary(lm(obs.mu ~mu))$r.squared,2)
mtext(paste0('R2=',rsq), side = 3, line = -2.7, adj = 0.03)
#add confidence interval.
range <- mu
...
#plot
plot(obs.mu ~ mu, cex = 0.7, ylim=c(0,.1), main='site-level')
