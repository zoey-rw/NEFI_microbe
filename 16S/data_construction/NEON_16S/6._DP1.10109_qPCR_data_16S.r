#Getting NEON qPCR data for 16S data construction.
#outputs table of fungal, bacterial and archaeal qPCR data.
#clearn environment, source output paths.
rm(list=ls())
source('paths.r')

dp1.10109.00_output_16S.path <- "/fs/data3/caverill/NEFI_data/16S/pecan_gen/NEON_data_aggregation/dp1.10109.00_output_16S.rds"
...
for(i in 1:nrow(site_date)){
  ...
  for(k in 1:length(dates)){
    ...
    bac_arc <- core.data[core.data$targetTaxonGroup == 'bacteria and archaea',]
    #subset, rename, merge together
    bac_arc <- bac_arc[,c('dnaSampleID','meanCopyNumber','copyNumberStandardDeviation')]
    names(fun)[names(fun) == 'meanCopyNumber'] <- 'fun.CopyNumber'
    names(bac_arc)[names(bac_arc) == 'meanCopyNumber'] <- 'bac_arc.CopyNumber'
    names(fun)[names(fun) == 'copyNumberStandardDeviation'] <- 'fun.copyNumberStandardDeviation'
    names(bac_arc)[names(bac_arc) == 'copyNumberStandardDeviation'] <- 'bac_arc.copyNumberStandardDeviation'
    site_date.output <- merge(fun,bac_arc, all = T)
...
saveRDS(output, dp1.10109.00_output_16S.path)
