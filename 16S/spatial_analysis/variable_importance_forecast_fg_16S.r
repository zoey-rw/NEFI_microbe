...
#set output path.----
output.path <- NEON_fg_variable_importance_data_16S.path

#load forecast.----
#forecast.
#dat <- readRDS(NEON_cps_fcast_fg_16S.path)
dat <- readRDS("/fs/data3/caverill/NEFI_data/16S/pecan_gen/NEON_forecast_data/NEON_cps_fcast_fg_16S_no.nutr.rds")
#prior.
mod <- readRDS(prior_16S_all.fg.groups_JAGSfits.path)
...
for(j in 1:length(mod)){
  ...
  group.mod <- mod[[j]]$no.nutr.preds
  mcmc <- group.mod$jags_model$mcmc
  mcmc <- do.call(rbind,mcmc)
  preds <- dat[[j]]$site.preds
  n.spp <- length(group.mod$species_parameter_output)
  to_sim <- pred[[j]]
  ...
  for(i in 1:length(output.list)){
    ...
    colnames(output.list[[i]]) <- names(group.mod$species_parameter_output)
  }
