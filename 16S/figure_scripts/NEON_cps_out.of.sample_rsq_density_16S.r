...
#set output path.----
if (compl_cases==T){ output.path <- paste0(pecan_gen_16S_dir,"/figures/valR2_phylo_compl_cases_density_fig.png")
} else output.path <- paste0(pecan_gen_16S_dir,"/figures/valR2_phylo_density_fig.png")
...
#plot.----
title <- ifelse(compl_cases==T, "Bacteria - R2 by spatial scale - no gap-filling", "Bacteria - R2 by spatial scale - gap-filled")
plot(core.d,xlim = c(0, 0.9), ylim = limy, bty = 'n', xlab = NA, ylab = NA, main = title, yaxs='i', xaxs = 'i', las = 1, lwd = 0)
