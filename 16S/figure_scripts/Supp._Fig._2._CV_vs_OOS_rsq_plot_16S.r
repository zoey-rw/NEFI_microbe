...
oos.cal <- readRDS(prior_delgado_ddirch_16S.path)
oos.val <- readRDS(NEON_cps_fcast_ddirch_16S.path)
...
for(i in 1:length(oos.val)){
  ...
  for(k in 1:length(y)){
    ...
    yy <- yy[rownames(xx),] # a simpler way for matching rows/cols?
...
for(i in 1:length(cv.val.plot)){
  y <- as.data.frame(cv.dat.plot[[i]]$mean)
  #match row and column names.
  x <- cv.val.plot[[i]]$plot.fit$mean
  y <- y[,order(match(colnames(y), colnames(x)))]
  x <- x[rownames(x) %in% rownames(y),]
  y <- y[rownames(x),] # a simpler way for matching rows/cols
  #Get R2 for each column.
  lev.stats <- list()
  for(j in 1:ncol(y)){
    #r square best fit.
