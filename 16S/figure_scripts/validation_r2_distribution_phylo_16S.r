...
output.path <- paste0(pecan_gen_16S_dir,"figures/valR2_by.phylo.fg_density_fig.png")
...
pl <- c(fg, pl) # combine fg and phylo fits
...
fg.r2 <- list(unlist(pl.r2[1:12]))
names(fg.r2) <- "functional group"
name.list <- c("functional group", "phylum", "class", "order", "family", "genus") # we'll use this later too
all.r2 <- c(fg.r2, pl.r2[13:17])
names(all.r2) <- name.list
...
pl.cast <- c(fg.cast, pl.cast)
pl.truth <- c(fg.truth, pl.truth)
pl.core <- c(fg.core, pl.core)
#Re-order.
...
# fix some naming issues - site level
fg.site <- list(unlist(all.site.rsq[1:12]))
all.site.rsq <- c(fg.site, all.site.rsq[13:17])
names(all.site.rsq) <- name.list
# plot level
fg.plot <- list(unlist(all.plot.rsq[1:12]))
all.plot.rsq <- c(fg.plot, all.plot.rsq[13:17])
names(all.plot.rsq) <- name.list
# core level
fg.core <- list(unlist(all.core.rsq[1:12]))
all.core.rsq <- c(fg.core, all.core.rsq[13:17])
names(all.core.rsq) <- name.list
...
#names(lev.mu) <- c("functional group", names(pl.cast)[13:17])
...
# validation r2 by phylo/function scale - with all data showing

df <- lapply(all.site.rsq, data.frame)
levs <- list()
for (i in 1:6){
  lev <- df[[i]]
