#spatial forecast variance decomposition: bacterial functional groups.
...
#set output path.----
output.path <- NEON_ddirch_var.decomp_fg_16S.path

#load model results.----
#mod 1 is data from maps.
#mod 2 is site-specific data, no maps.
#mod 3 is all covariates.
all_mod <- readRDS(prior_16S_all.fg.groups_JAGSfits.path)

all_fg <- list()
for (p in 1:length(all_mod)){

mod <- all_mod[[p]]    
mod <- mod[[1]] #just the no-nutr case.
